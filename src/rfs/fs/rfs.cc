                    ...
                    for (size_t i = 0; i < sizes.size(); i++) {
                        if (sizes[i] > 0) {
                            if (i < sizeof(devnames) / sizeof(*devnames))
                                ...
                                f->dump_int(("dev-" + to_string(i)).c_str(),
                                            sizes[i]);
                        }
                    }
                    f->close_section();
...
void BlueFS::_init_logger() {
    ...
    b.add_u64_counter(
        l_bluefs_read_random_buffer_bytes, "read_random_buffer_bytes",
...
void BlueFS::_init_alloc() {
    ...
    for (unsigned id = 0; id < bdev.size(); ++id) {
        ...
        if (is_shared_alloc(id)) {
            dout(1) << __func__ << " shared, id " << id << " alloc_size 0x"
                    << std::hex << alloc_size[id] << " size 0x"
                    << bdev[id]->get_size() << std::dec << dendl;
        } else {
            ...
            dout(1) << __func__ << " new, id " << id << " alloc_size 0x"
                    << std::hex << alloc_size[id] << " size 0x"
                    << bdev[id]->get_size() << std::dec << dendl;
            alloc[id] =
                Allocator::create(cct, cct->_conf->bluefs_allocator,
...
int BlueFS::mount() {
    ...
    int r = _open_super();
    if (r < 0) {
        derr << __func__ << " failed to open super: " << cpp_strerror(r)
             << dendl;
        goto out;
    }
    ...
    if (r < 0) {
        derr << __func__ << " failed to replay log: " << cpp_strerror(r)
             << dendl;
        _stop_alloc();
        goto out;
    }
    ...
    // init freelist
    for (auto& p : file_map) {
        dout(30) << __func__ << " noting alloc for " << p.second->fnode
                 << dendl;
        for (auto& q : p.second->fnode.extents) {
    ...
    if (shared_alloc) {
        shared_alloc->need_init = false;
        dout(1) << __func__
                << " shared_bdev_used = " << shared_alloc->bluefs_used << dendl;
    } else {
...
int BlueFS::maybe_verify_layout(const bluefs_layout_t& layout) const {
    if (super.memorized_layout) {
        if (layout == *super.memorized_layout) {
            dout(10) << __func__ << " bluefs layout verified positively"
                     << dendl;
        } else {
...
int BlueFS::_write_super(int dev) {
    ...
    dout(20) << __func__ << " v " << super.version << " crc 0x" << std::hex
             << crc << " offset 0x" << get_super_offset() << std::dec << dendl;
    return 0;
}
...
int BlueFS::_replay(bool noop, bool to_stdout) {
    ...
    if (!noop) {
        ...
        // do not use fnode from superblock in 'noop' mode - log_file's one
        // should be fine and up-to-date
        ...
        if (cct->_conf->bluefs_log_replay_check_allocations) {
            for (size_t i = 0; i < MAX_BDEV; ++i) {
                if (alloc_size[i] != 0 && bdev[i] != nullptr) {
                    used_blocks[i].resize(
                        round_up_to(bdev[i]->get_size(), alloc_size[i]) /
                        alloc_size[i]);
                }
            }
        }
    ...
    while (true) {
        ...
        bufferlist bl;
        {
            ...
            if (r != (int)super.block_size &&
                cct->_conf->bluefs_replay_recovery) {
                r += do_replay_recovery_read(log_reader, pos, read_pos + r,
            ...
            if (r < (int)more) {
                ...
                if (cct->_conf->bluefs_replay_recovery) {
                    ...
                    r += do_replay_recovery_read(log_reader, pos, read_pos + r,
                                                 more - r, &t);
                    if (r < (int)more) {
                        // in normal mode we must read r==more, for recovery it
                        // is too strict
                        break;
                    }
                }
            ...
            switch (op) {
                ...
                case bluefs_transaction_t::OP_JUMP: {
                    ...
                    if (skip) {
                        ...
                        if (r != (int)skip) {
                            dout(10) << __func__ << " 0x" << std::hex
                                     << read_pos << ": stop: failed to skip to "
                    ...
                    if (!noop) {
                        ...
                        if (cct->_conf->bluefs_log_replay_check_allocations) {
                            ...
                            if (first_log_check) {
                                ...
                                int r = _check_new_allocations(
                                    log_file->fnode, MAX_BDEV, used_blocks);
                                if (r < 0) {
                                    return r;
                                }
                            }
                            ...
                            for (auto e : fnode_extents) {
                                auto id = e.bdev;
                                if (int r = _verify_alloc_granularity(
                                        id, e.offset, e.length,
                                        "OP_FILE_UPDATE");
                                    r < 0) {
                                    return r;
                                }
                                apply_for_bitset_range(
                                    e.offset, e.length, alloc_size[id],
                                    used_blocks[id],
                                    [&](uint64_t pos,
                        ...
                        if (cct->_conf->bluefs_log_replay_check_allocations) {
                            int r = _check_new_allocations(f->fnode, MAX_BDEV,
                                                           used_blocks);
                            if (r < 0) {
                                return r;
                            }
                ...
                case bluefs_transaction_t::OP_FILE_REMOVE: {
                    ...
                    if (!noop) {
                        ...
                        vselector->sub_usage(p->second->vselector_hint,
                                             p->second->fnode);
                        if (cct->_conf->bluefs_log_replay_check_allocations) {
                            ...
                            for (auto e : fnode_extents) {
                                ...
                                if (fail) {
                                    derr << __func__ << " invalid extent "
                                         << int(id) << ": 0x" << std::hex
                                         << e.offset << "~" << e.length
                                         << std::dec
                                         << ": not in use but is allocated for "
                                            "removed ino "
                                         << ino << dendl;
                                    return -EFAULT;
                                }
    ...
    if (!noop && first_log_check &&
        ...
        int r = _check_new_allocations(log_file->fnode, MAX_BDEV, used_blocks);
        if (r < 0) {
            return r;
        }
    }
    ...
    if (!noop) {
        ...
        for (auto& p : file_map) {
            if (p.second->refs == 0 && p.second->fnode.ino > 1) {
                derr << __func__
                     << " file with link count 0: " << p.second->fnode << dendl;
                return -EIO;
            }
        }
    }
...
int BlueFS::device_migrate_to_existing(CephContext* cct,
    ...
    if ((flags & REMOVE_DB) && dev_target == BDEV_SLOW) {
        ...
        dout(0) << __func__ << " super to be written to " << dev_target
                << dendl;
    }
    ...
    for (auto& [ino, file_ref] : file_map) {
        ...
        bool rewrite = std::any_of(
            fnode_extents.begin(), fnode_extents.end(), [=](auto& ext) {
        ...
        if (rewrite) {
            ...
            for (auto old_ext : fnode_extents) {
                ...
                int r = bdev[old_ext.bdev]->read_random(
                    old_ext.offset, old_ext.length, (char*)&buf.at(0),
                    buffered);
                if (r != 0) {
            ...
            auto l =
                _allocate_without_fallback(dev_target, bl.length(), &extents);
            if (l < 0) {
            ...
            for (auto& i : extents) {
                bufferlist cur;
                uint64_t cur_len =
                    std::min<uint64_t>(i.length, bl.length() - off);
            ...
            for (auto& ext : fnode_extents) {
                if (dev_target != dev_target_new && ext.bdev == dev_target) {
                    dout(20) << __func__ << "  "
                             << " ... adjusting extent 0x" << std::hex
                             << ext.offset << std::dec << " bdev " << dev_target
    ...
    int new_log_dev_cur =
        bdev[BDEV_WAL] ? BDEV_WAL : bdev[BDEV_DB] ? BDEV_DB : BDEV_SLOW;
...
int BlueFS::device_migrate_to_new(CephContext* cct, const set<int>& devs_source,
    ...
    for (auto& p : file_map) {
        ...
        dout(10) << __func__ << " " << p.first << " " << p.second->fnode
                 << dendl;
    ...
    int new_log_dev_next =
        new_log_dev_cur == BDEV_NEWWAL
            ? BDEV_WAL
            : new_log_dev_cur == BDEV_NEWDB ? BDEV_DB : new_log_dev_cur;

    int super_dev = dev_target == BDEV_NEWDB
                        ? BDEV_NEWDB
...
BlueFS::FileRef BlueFS::_get_file(uint64_t ino) {
    ...
    if (p == file_map.end()) {
        ...
        file_map[ino] = f;
        dout(30) << __func__ << " ino " << ino << " = " << f << " (new)"
                 << dendl;
        return f;
    } else {
                             ...
                             char* out)      ///< [out] copy it here
{
    ...
    while (len > 0) {
        if (off < buf->bl_off || off >= buf->get_buf_end()) {
            ...
            dout(20) << __func__ << " read random 0x" << std::hex << x_off
            ...
            dout(20) << __func__ << " left 0x" << std::hex << left << " 0x"
            ...
            dout(30) << __func__ << " result chunk (0x" << std::hex << r
                     << std::dec << " bytes):\n";
            bufferlist t;
    ...
    char* out)          ///< [out] optional: or copy it here
{
    ...
    // 如果没有outbuffer, 以及输出，那么就是预读
    bool prefetch = !outbl && !out;

    // 如果已经越界了，那么直接把Len = 0设置好。
    if (!h->ignore_eof && off + len > h->file->fnode.size) {
        if (off > h->file->fnode.size)
            len = 0;
        else
...
void BlueFS::_compact_log_async(std::unique_lock<ceph::mutex>& l) {
    ...
    dout(10) << __func__ << " old_log_jump_to 0x" << std::hex << old_log_jump_to
             << " need 0x"
             << (old_log_jump_to + cct->_conf->bluefs_max_log_runway)
             << std::dec << dendl;
    int r = _allocate(vselector->select_prefer_bdev(log_file->vselector_hint),
                      cct->_conf->bluefs_max_log_runway, &log_file->fnode);
    ceph_assert(r == 0);
    ...
    while (discarded < old_log_jump_to) {
        ...
        if (discarded + e.length <= old_log_jump_to) {
            ...
            dout(10) << __func__ << " remove front of old log extent " << e
                     << dendl;
            ...
            e.length -= drop;
            discarded += drop;
            dout(10) << __func__ << "   kept " << e << " removed " << temp
                     << dendl;
        }
...
int BlueFS::_flush_and_sync_log(std::unique_lock<ceph::mutex>& l,
    ...
    if (want_seq && want_seq <= log_seq_stable) {
        dout(10) << __func__ << " want_seq " << want_seq
                 << " <= log_seq_stable " << log_seq_stable << ", done"
                 << dendl;
        ceph_assert(!jump_to);
        return 0;
    }
    ...
    if (just_expanded_log) {
        ceph_assert(bl.length() <= runway);  // if we write this, we will have
    ...
    if (seq > log_seq_stable) {
        ...
        while (p != dirty_files.end()) {
            if (p->first > log_seq_stable) {
                dout(20) << __func__ << " done cleaning up dirty files"
                         << dendl;
                break;
            }
            ...
            while (l != p->second.end()) {
                ...
                ceph_assert(file->dirty_seq <= log_seq_stable);
                dout(20) << __func__ << " cleaned file " << file->fnode
                         << dendl;
                file->dirty_seq = 0;
                p->second.erase(l++);
            }
    ...
    for (unsigned i = 0; i < to_release.size(); ++i) {
        if (!to_release[i].empty()) {
            ...
            if (cct->_conf->bdev_enable_discard &&
                ...
                for (auto p = to_release[i].begin(); p != to_release[i].end();
                     ++p) {
...
int BlueFS::_flush_range(FileWriter* h, uint64_t offset, uint64_t length) {
    ...
    if (allocated < offset + length) {
        ...
        int r =
            _allocate(vselector->select_prefer_bdev(h->file->vselector_hint),
                      offset + length - allocated, &h->file->fnode);
        if (r < 0) {
            derr << __func__ << " allocated: 0x" << std::hex << allocated
                 << " offset: 0x" << offset << " length: 0x" << length
                 << std::dec << dendl;
    ...
    if (partial) {
        ...
        offset -= partial;
        length += partial;
        dout(20) << __func__ << " waiting for previous aio to complete"
                 << dendl;
        for (auto p : h->iocv) {
            if (p) {
                p->aio_wait();
...
void BlueFS::wait_for_aio(FileWriter* h) {
    ...
    *_dout << " " << h << dendl;
    ...
    dout(10) << __func__ << " " << h << " done in "
             << (ceph_clock_now() - start) << dendl;
}
#endif

int BlueFS::_flush(FileWriter* h, bool force,
                   std::unique_lock<ceph::mutex>& l) {
    bool flushed = false;
    ...
    if (alloc_len < 0 || alloc_len < (int64_t)left) {
        ...
        if (bdev[id])
            derr << __func__ << " failed to allocate 0x" << std::hex << left
                 ...
                 << alloc[id]->get_free() << std::dec << dendl;
        else
            derr << __func__ << " failed to allocate 0x" << std::hex << left
                 << " on bdev " << (int)id << ", dne" << std::dec << dendl;
        if (alloc[id]) alloc[id]->dump();
        return -ENOSPC;
    }
    if (is_shared_alloc(id)) {
...
int BlueFS::_allocate(uint8_t id, uint64_t len, bluefs_fnode_t* node) {
    ...
    if (alloc[id]) {
        ...
        extents.reserve(
            4);  // 4 should be (more than) enough for most allocations
    ...
    if (!alloc[id] || alloc_len < 0 ||
        ...
        if (id != BDEV_SLOW) {
            if (bdev[id]) {
                dout(1) << __func__ << " failed to allocate 0x" << std::hex
                        << len << " on bdev " << (int)id << ", free 0x"
                        << alloc[id]->get_free() << "; fallback to bdev "
                        << (int)id + 1 << std::dec << dendl;
            }
...
void BlueFS::sync_metadata(bool avoid_compact) {
    ...
    if (log_t.empty() && dirty_files.empty()) {
        ...
        *_dout << dendl;
        flush_bdev();  // FIXME?
        _flush_and_sync_log(l);
        dout(10) << __func__ << " done in " << (ceph_clock_now() - start)
                 << dendl;
    }
...
int BlueFS::open_for_write(const string& dirname, const string& filename,
    ...
    if (p == dir_map.end()) {
        ...
        dout(20) << __func__ << "  dir " << dirname << " does not exist"
                 << dendl;
        return -ENOENT;
    } else {
        dir = p->second;
    ...
    if (q == dir->file_map.end()) {
        if (overwrite) {
            dout(20) << __func__ << " dir " << dirname << " (" << dir
                     << ") file " << filename << " does not exist" << dendl;
            return -ENOENT;
        }
...
int BlueFS::open_for_read(const string& dirname, const string& filename,
    ...
    // 如果是随机读，那么就是4KB，如果可以预读1MB
    const int random_prefetch_size = 4096;
    const int bluefs_max_prefetch = 1048576;
    *h = new FileReader(file,
                        random ? random_prefetch_size : bluefs_max_prefetch,
                        random, false /*ignore_end_of_file*/);
 ...
 */
int BlueFS::do_replay_recovery_read(FileReader* log_reader, size_t replay_pos,
                                    size_t read_offset, size_t read_len,
                                    bufferlist* bl) {
    dout(1) << __func__ << " replay_pos=0x" << std::hex << replay_pos
            << " needs 0x" << read_offset << "~" << read_len << std::dec
            << dendl;
    ...
    struct compare {
        bool operator()(const bluefs_extent_t& a,
                        const bluefs_extent_t& b) const {
    ...
    for (int dcnt = 0; dcnt < 3; dcnt++) {
        ...
        dout(5) << __func__ << " " << disk_regions_count
                << " regions to scan on " << get_device_name(dev) << dendl;
        ...
        for (size_t i = 0; i < disk_regions_count; reg++, i++) {
            ...
            std::unique_ptr<char[]> raw_data_p{
                new char[page_size + chunk_size]};
            ...
            while (len > last_32.length()) {
                ...
                dout(5) << __func__ << " read " << get_device_name(dev) << ":0x"
                        << std::hex << pos << "+" << chunk_len << std::dec
                        << dendl;
                r = bdev[dev]->read_random(pos, chunk_len, raw_data + page_size,
                                           cct->_conf->bluefs_buffered_io);
                ceph_assert(r == 0);
                ...
                for (char* sp = search_b;; sp += last_32.length()) {
                    ...
                    bufferlist test;
                    test.append(n,
                                std::min<size_t>(max_extent_size, chunk_e - n));
                    bluefs_extent_t ne;
                    try {
                    ...
                    if (extents_rejected.count(ne) != 0) {
                        dout(5) << __func__ << " extent " << ne
                                << " already refected" << dendl;
                        continue;
                    }
                    // insert as rejected already. if we succeed, it wouldn't
                    // make difference.
                    ...
                    if (ne.bdev >= MAX_BDEV || bdev[ne.bdev] == nullptr ||
                        ...
                        (ne.offset & 4095) != 0) {
                        dout(5)
                            << __func__ << " refusing extent " << ne << dendl;
                        continue;
                    }
                    ...
                    r = bdev[ne.bdev]->read(ne.offset, ne.length, &candidate,
                                            ioc[ne.bdev],
                                            cct->_conf->bluefs_buffered_io);
                    ceph_assert(r == 0);
