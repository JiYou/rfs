...
class BlueFS {
   public:
    ...
    struct FileWriter {
       ...
       public:
        ...
        void append(ceph::buffer::list& bl) { buffer.claim_append(bl); }

        void append_zero(size_t len) { buffer_appender.append_zero(len); }
    ...
    struct FileReader {
        ...
        bool random;
        // 这个在读自己的用户态的文件系统的时候需要
        bool ignore_eof;

        ceph::shared_mutex lock{
            ceph::make_shared_mutex(std::string(), false, false, false)};

        FileReader(FileRef f, uint64_t max_pre_fetch_size, bool is_random,
                   bool ignore_end_of_file)
            : file(f),
              buf(max_pre_fetch_size),
              random(is_random),
              ignore_eof(ignore_end_of_file) {
            ++file->num_readers;
        }
   ...
   private:
    ...
    void _pad_bl(ceph::buffer::list&
                     bl);  ///< pad ceph::buffer::list to block size w/ zeros
    ...
    void _rewrite_log_and_layout_sync(bool allocate_with_fallback,
                                      int super_dev, int log_dev,
                                      int new_log_dev, int flags,
    ...
    int64_t _read(FileReader* h,    ///< [in] read from here
                  ...
                  ceph::buffer::list*
                      outbl,   ///< [out] optional: reference the result here
   ...
   public:
    ...
    int device_migrate_to_new(CephContext* cct,
                              const std::set<int>& devs_source, int dev_target,
                              const bluefs_layout_t& layout);
    int device_migrate_to_existing(CephContext* cct,
    ...
    int open_for_write(const std::string& dir, const std::string& file,
                       FileWriter** h, bool overwrite);

    int open_for_read(const std::string& dir, const std::string& file,
    ...
    int lock_file(const std::string& dirname, const std::string& filename,
                  FileLock** p);
    ...
    int add_block_device(
        unsigned bdev, const std::string& path, bool trim, uint64_t reserved,
    ...
    int do_replay_recovery_read(FileReader* log, size_t log_pos,
