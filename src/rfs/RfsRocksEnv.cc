...
rocksdb::Status err_to_status(int r) {
    switch (r) {
        ...
        default:
            printf(stderr, "get error code = %d, not supported", r);
...
class BlueRocksRandomAccessFile : public rocksdb::RandomAccessFile {
   ...
   public:
    ...
    // Furthermore, it tries to make this ID at most "max_size" bytes. If such
    // an ID can be created this function returns the length of the ID and
    // places it in "id"; otherwise, this function returns 0, in which case "id"
    // may not have been modified.
    //
    // This function guarantees, for IDs from a given environment, two unique
    // ids cannot be made equal to eachother by adding arbitrary bytes to one of
    // them. That is, no unique ID is the prefix of another.
    //
    // This function guarantees that the returned ID will not be interpretable
    // as a single varint.
    //
...
rocksdb::Status BlueRocksEnv::NewSequentialFile(
    ...
    // 如果这个文件是向非user file system要文件
    // 那么直接向posix文件系充要内容
    // 这里的target()就是指向了posix文件系统
    if (fname[0] == '/')
...
rocksdb::Status BlueRocksEnv::ReuseWritableFile(
    const std::string& new_fname, const std::string& old_fname,
...
rocksdb::Status BlueRocksEnv::NewDirectory(
    const std::string& name, std::unique_ptr<rocksdb::Directory>* result) {
...
rocksdb::Status BlueRocksEnv::NewLogger(
    const std::string& fname, std::shared_ptr<rocksdb::Logger>* result) {
    // ignore the filename :)
